...
def saveImage(url, name):
    ...
    if len(list(Path(f'{IMAGE_BASE_FOLDER}/{name}').glob('*.jpg'))) == 0:   # check if folder contain image file
        save_img = urllib.request.urlretrieve(url, f'{IMAGE_BASE_FOLDER}/{name}/0000.jpg')  # save first image
    else:
        ...
        save_img = urllib.request.urlretrieve(url, f'{IMAGE_BASE_FOLDER}/{name}/{next_img}.jpg') # save next image
